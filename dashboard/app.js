...
const bO = (fmt = 'czk') => ({
    ...
    plugins: {
        legend: { labels: { color: '#9ca3af', font: { size: 11, family: 'Inter' }, padding: 14, usePointStyle: true, pointStyle: 'circle' } },
        tooltip: {
            ...
            callbacks: {
                label: ctx => {
                    ...
                    if (fmt === 'pct') return l + ': ' + v.toFixed(1).replace('.', ',') + ' %';
                    if (fmt === 'days') return l + ': ' + Math.round(v) + ' dní';
                    return l + ': ' + CZK.format(Math.round(v));
                }
            }
        }
    },
    scales: {
        x: { ticks: { color: '#6b7280', font: { size: 10, family: 'Inter' } }, grid: { color: 'rgba(75,85,99,0.12)' } },
        y: {
            ticks: {
                color: '#6b7280',
